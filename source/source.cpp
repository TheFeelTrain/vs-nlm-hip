...
static void hipFreeCustom(void * p) {
    showError(hipFree(p));
}

static void hipFreeHostCustom(void * p) {
    showError(hipHostFree(p));
}
...
struct NLMeansStreamData {
    ...
    int buffer_stride;
    int image_stride;
    Resource<uint8_t *, hipFreeHostCustom> h_buffer;
    Resource<void *, hipFreeCustom, true> d_src_ref;
    Resource<float *, hipFreeCustom, true> d_buffer;
    Resource<float *, hipFreeCustom, true> d_buffer_bwd;
    Resource<float *, hipFreeCustom, true> d_buffer_fwd; // may be empty
    Resource<float *, hipFreeCustom, true> d_wdst;
    Resource<float *, hipFreeCustom, true> d_weight;
    Resource<float *, hipFreeCustom, true> d_max_weight;
    Resource<void *, hipFreeCustom, true> d_dst;
};
...
static const VSFrame *VS_CC NLMeansGetFrame(
    ...
    checkError(hipSetDevice(d->device_id));
...

    {
        ...
        for (const auto & src_frame : src_frames) {
            for (int plane = 0; plane < 3; plane++) {
                ...
                ptrdiff_t pitch = vsapi->getStride(src_frame.get(), plane);
...

    {
        ...
        for (int plane = 0; plane < 3; plane++) {
            ...
            ptrdiff_t pitch = vsapi->getStride(dst_frame.get(), plane);
...
static void VS_CC NLMeansCreate(
    ...
    d->radius = vsh::int64ToIntS(vsapi->mapGetInt(in, "d", 0, &error));
    if (error) {
        d->radius = 1;
    }
    if (d->radius < 0) {
    ...
    d->spatial_radius = vsh::int64ToIntS(vsapi->mapGetInt(in, "a", 0, &error));
    if (error) {
        d->spatial_radius = 2;
    }
    ...
    d->block_radius = vsh::int64ToIntS(vsapi->mapGetInt(in, "s", 0, &error));
    if (error) {
        d->block_radius = 4;
    }
    if (d->block_radius < 0) {
    ...
    auto h = vsapi->mapGetFloat(in, "h", 0, &error);
    if (error) {
        h = 1.2;
    }
    if (h <= 0.0) {
    ...
    auto channels = vsapi->mapGetData(in, "channels", 0, &error);
    if (error) {
        channels = "AUTO";
    }
    ...
    if (d->channels == ChannelMode::Y) {
        ...
        if (vi->format.colorFamily != cfYUV || vi->format.subSamplingW || vi->format.subSamplingH) {
    ...
    d->wmode = static_cast<int>(vsapi->mapGetInt(in, "wmode", 0, &error));
    if (error) {
        d->wmode = 0;
    }
    ...
    d->wref = static_cast<float>(vsapi->mapGetFloat(in, "wref", 0, &error));
    if (error) {
        d->wref = 1.0f;
    }
    ...
    d->device_id = vsh::int64ToIntS(vsapi->mapGetInt(in, "device_id", 0, &error));
    if (error) {
        d->device_id = 0;
    }
    ...
    for (int i = 0; i < num_streams; i++) {
        ...
        checkError(hipMalloc(&stream_data.d_dst.data, filter_num_planes * filter_height * image_pitch));

        size_t buffer_pitch;
        checkError(hipMallocPitch(&stream_data.d_max_weight.data, &buffer_pitch, filter_width * sizeof(float), filter_height));
        stream_data.buffer_stride = static_cast<int>(buffer_pitch / sizeof(float));

        checkError(hipMalloc(&stream_data.d_buffer.data, filter_height * buffer_pitch));
        checkError(hipMalloc(&stream_data.d_buffer_bwd.data, filter_height * buffer_pitch));
        if (d->radius > 0) {
            checkError(hipMalloc(&stream_data.d_buffer_fwd.data, filter_height * buffer_pitch));
        }
        checkError(hipMalloc(&stream_data.d_wdst.data, filter_num_planes * filter_height * buffer_pitch));
        checkError(hipMalloc(&stream_data.d_weight.data, filter_height * buffer_pitch));
        ...
        checkError(hipHostMalloc(&stream_data.h_buffer.data, h_buffer_size));
    }

    VSFilterDependency deps[2] = {{d->node, rpGeneral}};
    int num_deps = 1;
    if (d->ref_node) {
        deps[num_deps++] = {d->ref_node, rpGeneral};
    }
